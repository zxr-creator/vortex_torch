#!/usr/bin/env bash
# Sweep decode profiling over batch sizes and input lengths -- DENSE baseline.
# Runs profile_decode.py with --no-vortex (plain FlashInfer dense attention),
# generating .nsys-rep + readable nsys stats reports for each setting.
...
OUT_DIR=${OUT_DIR:-nsys_decode_reports_dense}
RUN_TAG=${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}
# Set PROFILE_PREFILL=1 to also capture prefill-time kernels.
...
echo "run_tag,attn_label,model_path,model_label,batch_size,input_len,max_new_tokens,nsys_rep,kern_sum,trace_report,nvtx_report,log_file,status" > "${MANIFEST}"
...
for midx in "${!MODEL_PATHS[@]}"; do
  ...
  for bs in "${BATCH_SIZES[@]}"; do
    for idx in "${!INPUT_LENS[@]}"; do
      ...
      echo "${RUN_TAG},${attn_label},${model_path},${model_label},${bs},${input_len},${MAX_NEW_TOKENS},${nsys_rep},${kern_sum_report},${trace_report},${nvtx_report},${log_file},${status}" >> "${MANIFEST}"
    done
  done
done
