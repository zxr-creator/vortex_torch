 ...
 */
 ...
 // allocations inside the transform kernels depend on it. Set to 32 to
 // match the smallest --topk-val in bench_topk_kernels.sh; rebuild with
 // a different value (and update the TORCH_CHECK in topk_output_sglang_ori)
 // to bench other K. The transform kernels (decode/prefill/prefill_ragged)
 // still carry a manual unroll that assumes TopK==2048; that code path is
 // unreachable from the bench (we only invoke fast_topk_interface), so the
 // corresponding static_asserts have been removed below.
    ...
    const int64_t     max_num_pages)
{
    ...
    TORCH_CHECK(topk_val == static_cast<int64_t>(32),
                "topk_output_sglang_ori: this build of the ori kernel hard-codes TopK=32; "
                "rebuild sglang_topk.cu with a different TopK if you need another value. "
                "Got topk_val=", topk_val);
    TORCH_CHECK(indices_out.dim() == 2
                    && indices_out.size(0) == eff_batch_size
                    && indices_out.size(1) == 32,
                "indices_out must be [eff_batch_size, 32]");
