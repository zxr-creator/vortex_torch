...
# is locked in.
# Bumped from 5/20 to 15/80. With only 20 timed iters, run-to-run noise in
# CUDA event timing was large enough that mappings 1-2% slower than the
# best looked sometimes faster (e.g., TANH@1.0 picked over SHIFT_POW2@0.5
# at uniform L=131k, then ran 13% slower in the actual bench). 80 timed
# iters cuts the noise floor substantially while keeping autotune total
# time manageable (~25 s across the full sweep).
AUTOTUNE_WARMUP = 15
AUTOTUNE_ITERS  = 80
...
def autotune_remap(
    ...
    # Latency-first: among any candidate that clears the ABS recall floor,
    # rank by lowest mean latency. When the TARGET tier (≥ 0.99) is empty
    # the previous behavior was to fall back to *highest recall*, which
    # made the autotune pick a slow-but-marginally-more-accurate mapping
    # like TANH@1.0 (0.987 recall, 597 µs) over the genuinely-faster
    # SHIFT_POW2@0.5 (0.976 recall, 472 µs) at uniform L=131k. We always
    # rank by latency now, with the ABS floor (0.95) as the only
    # correctness gate; a high-recall slower option only wins if NO
    # candidate clears ABS.
    if correct:
        pool_sorted = sorted(correct, key=lambda r: r[5])
        # Reconfirm (below) only fires in this latency-driven path.
        is_latency_pick = True
    else:
        # Nothing clears the ABS floor — fall back to highest-recall option.
        pool_sorted = sorted(measured, key=lambda r: -r[6])
        is_latency_pick = False

    # Top-3 reconfirm at full bench iters. Autotune timing (15 warmup +
    # 80 iters) is noise-prone enough at sub-microsecond resolution that
    # a candidate genuinely 10-20% slower can occasionally clock in 1-2%
    # faster than the real best (e.g., TANH@1.0 vs SHIFT_POW2@0.5 at
    # uniform L=131k). Re-time the top 3 candidates with the same iter
    # count the bench will use for the final timing — that picks up
    # systematic differences and stabilises the choice.
    RECONFIRM_WARMUP = 20
    RECONFIRM_ITERS  = 100
    top_n = pool_sorted[:3]
    if len(top_n) > 1 and is_latency_pick:
        reconfirmed = []
        for r in top_n:
            tag_full, tag, mode, power, tol, _, _ = r
            try:
                fn = (kernel_factory(mode, power, tol) if sweep_tol
                      else kernel_factory(mode, power))
                stats2 = time_kernel(fn, RECONFIRM_WARMUP, RECONFIRM_ITERS)
                # keep recall from the autotune measurement
                reconfirmed.append((tag_full, tag, mode, power, tol,
                                    stats2["mean_ms"], r[6]))
            except RuntimeError:
                continue
        if reconfirmed:
            best = min(reconfirmed, key=lambda r: r[5])
        else:
            best = pool_sorted[0]
    else:
        best = pool_sorted[0]
